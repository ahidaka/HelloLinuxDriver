 ...
 */
static int kthtest_set_sw(const char *val, const struct kernel_param *kp)
{
        if (val) {
 ...
 */
static int kthtest_get_sw(char *buffer, const struct kernel_param *kp)
{
	printk("%s:get: val = <%d>\n", kp->name, *((int *)kp->arg));
...
static const struct kernel_param_ops kthtest_sw_ops = {
        .set    = kthtest_set_sw,
        .get    = kthtest_get_sw,
};
module_param_cb(sw, &kthtest_sw_ops, &sw, 0644);
