...
static void deferred_print(struct work_struct *data)
{
	struct wq *wp = &wq2_data;
 ...
 */
int wq2_set(const char *val, const struct kernel_param *kp)
{
 ...
 */
int wq2_get(char *buffer, const struct kernel_param *kp)
{
	int l = *((int *)kp->arg);
	struct wq *datap = &wq2_data;
...
static const struct kernel_param_ops wq2_ops = {
        .set    = wq2_set,
        .get    = wq2_get,
};
module_param_cb(sw, &wq2_ops, &sw, 0644);
